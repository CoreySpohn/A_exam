...
if __name__ == '__main__':
    ...
    if failure == 'n_failure':
        ...
        a = 1.5*u.AU
        I = 90*u.degree
    elif failure == 'adet_no_failure':
        img_times = Time(np.linspace(2004, 2006, 365*2), format='decimalyear')
        pop_types = ['ML']
        rv_error = 1*u.m/u.s
        a = 1*u.AU
        I = 170*u.degree
    ...
    for fnum, current_time in enumerate( tqdm( img_times ) ):
        ...
        if plot_df:
            ...
            if len(df_ids) is 0:
                ...
                pvt_ax.scatter(plt_df_times, plt_df_vals, marker='d', label="Dispersion Failure", zorder=3)
