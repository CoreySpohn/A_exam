...
\section{Radial velocity (RV)}
\subsection{Why use RV} %note that you need both sections and subsection for the navigation to look good for this 
...
\frame{
    \frametitle{Questions for using RV data for imaging}
...
\section{RV fitting}%
\subsection{RV fitting}
\frame{
\frametitle{Completeness}
\tikzoverlay[text width = 14cm] at (0cm, 2.5cm+4.5pt){
\begin{enumerate}
    \item[\textbullet] ``The fraction of possible planets that will be detected for a given star''
        \parencite{Brown2005d}
    \item[\textbullet] 
    \item[\textbullet] Convert software outputs for use in direct imaging 
    \item[\textbullet] Analyze when the the conversion is harmful or useless
\end{enumerate}
};
}
