...
\footlineextra{Data from NExScI}
}
...
\tikzoverlay[text width = 14cm] at (0cm, 2.2cm+4.5pt){
    %\animategraphics[controls, width=14cm]{10}{rv_example/frame-}{0}{1}
...
%\end{itemize}
%};
\tikzoverlay[text width = 6cm] at (0cm, 2.2cm+4.5pt){
    %\animategraphics[controls, width=14cm]{20}{error_example/frame-}{0}{1}
...
\tikzoverlay[text width = 6cm] at (1cm, 3cm+4.5pt){
    %\animategraphics[loop, controls, width=12cm]{10}{mass_inclination_comparision/frame-}{0}{1}
...
\footlineextra{\hyperlink{coronagraphbackup}{\beamerreturnbutton{Backup}}}
}
...
\footlineextra{\hyperlink{lambertbackup}{\beamerreturnbutton{Backup}}}
}
...
\tikzoverlay[text width = 7cm] at (0cm-3pt, 2.2cm+4.5pt){
    %\animategraphics[controls, width=14cm]{10}{pdet/frame-}{0}{1}
...
\frame[label=radvelmain]{
\frametitle{Fitting RV curves}
\framesubtitle{ \hyperlink{radvelbackup}{\beamerreturnbutton{Backup}} }
\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
\begin{enumerate}
...
\tikzoverlay[text width = 10cm] at (0cm-3pt, 3.1cm+4.5pt){
    %\animategraphics[loop, controls, width=14cm]{20}{ML_vis/frame-}{0}{1}
...
\tikzoverlay[text width = 10cm] at (0cm-3pt, 3.1cm+4.5pt){
    %\animategraphics[loop, controls, width=14cm]{20}{MKDE_vis/frame-}{0}{1}
...
\tikzoverlay[text width = 10cm] at (0cm-3pt, 3.1cm+4.5pt){
    %\animategraphics[loop, controls, width=14cm]{20}{CI_vis/frame-}{0}{1}
...
\tikzoverlay[text width = 10cm] at (0cm-3pt, 3.1cm+4.5pt){
    %\animategraphics[loop, controls, width=14cm]{20}{MG_vis/frame-}{0}{1}
...
%\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
    ...
    %\end{itemize}
%};
\tikzoverlay[text width = 14cm] at (1cm, 3cm+4.5pt){
\includegraphics[width=12cm]{ratio_example/frame-99}
};
}

\frame[label=lambert2]{
...
\tikzoverlay[text width = 10cm] at (0cm-3pt, 2.6cm+4.5pt){
    %\animategraphics[loop, controls, width=14cm]{100}{CI_if/ratio-0.50-frame-}{0}{1}
...
\frame[label=intermittent_failure_main]{
    \frametitle{Intermittent failure example}
\tikzoverlay[text width = 14cm] at (0cm, 2.6cm+4.5pt){
    %\animategraphics[controls, width=14cm]{100}{ML_if/ratio-0.50-frame-}{0}{1}
    \animategraphics[controls, width=14cm]{100}{ML_if/ratio-0.50-frame-}{0}{364}
};
\footlineextra{\hyperlink{intermittent_failure_rules}{\beamerbutton{Rules}}}
}
...
%};
\tikzoverlay[text width = 14cm] at (0cm, 2.6cm+4.5pt){
    %\animategraphics[controls, width=14cm]{200}{CI_if/ratio-1.00-frame-}{0}{1}
    \animategraphics[controls, width=14cm]{200}{CI_if/ratio-1.00-frame-}{0}{999}
};
}

\frame{
    \frametitle{No failure, always detectable}
%\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
%\begin{enumerate}
    %\item[1)] Probability of detection curve flattens to the point where a full orbital period passes without any extrema
%\end{enumerate}
%};
\tikzoverlay[text width = 14cm] at (0cm, 2.6cm+4.5pt){
    %\animategraphics[controls, width=14cm]{100}{ML_if/ratio-1.00-frame-}{0}{1}
    \animategraphics[controls, width=14cm]{100}{ML_if/ratio-1.00-frame-}{0}{599}
};
}

\frame{
    \frametitle{Max likelihood results}
\tikzoverlay[text width = 10cm] at (8cm-3pt, 3.0cm+4.5pt){
\includegraphics[height=7cm]{./figures/ML_results.png}
};
\tikzoverlay[text width = 10cm] at (0cm-3pt, 1.0cm+4.5pt){
\includegraphics[width=7cm]{./figures/bar_plot_legend.png}
};
}

\frame{
    \frametitle{Max KDE results}
\tikzoverlay[text width = 10cm] at (8cm-3pt, 3.0cm+4.5pt){
\includegraphics[height=7cm]{./figures/MKDE_results.png}
};
\tikzoverlay[text width = 10cm] at (0cm-3pt, 1.0cm+4.5pt){
\includegraphics[width=7cm]{./figures/bar_plot_legend.png}
};
...
\frame{
    \frametitle{Credible interval results}
\tikzoverlay[text width = 10cm] at (8cm-3pt, 3.0cm+4.5pt){
\includegraphics[height=7cm]{./figures/CI_results.png}
};
\tikzoverlay[text width = 10cm] at (0cm-3pt, 1.0cm+4.5pt){
\includegraphics[width=7cm]{./figures/bar_plot_legend.png}
};
...
\frame{
    \frametitle{Multivariate Gaussian results}
\tikzoverlay[text width = 10cm] at (8cm-3pt, 3.0cm+4.5pt){
\includegraphics[height=7cm]{./figures/MG_results.png}
};
\tikzoverlay[text width = 10cm] at (0cm-3pt, 1.0cm+4.5pt){
\includegraphics[width=7cm]{./figures/bar_plot_legend.png}
};
...
\frame{
    \frametitle{Construction method analysis}
...
\tikzoverlay[text width = 10cm] at (8.5cm-3pt, 3cm+4.5pt){
\includegraphics[height=7cm]{./figures/construction_method_bars_including_always_detectable.pdf}
};
}

\frame{
    \frametitle{Intermittent failure metrics}
\tikzoverlay[text width = 14cm] at (0cm, 2.1cm+4.5pt){
\includegraphics[width=14cm]{./figures/ML_if/if_only_ratio-0.50-frame-364.png}
};
}

\frame{
    \frametitle{Intermittent failure analysis}
...
\frametitle{Outline}
\begin{itemize}
    \item[\textbullet] Improvements on current work
        \begin{itemize}
            \item[\textbullet] Photometry
            \item[\textbullet] Investigate failure modes improvements
            \item[\textbullet] Generalize the multivariate Gaussian method for better low ratio
                performance
        \end{itemize}
    \item[\textbullet] Verification using EXOSIMS
    \item[\textbullet] Multi-planet system handling
\end{itemize}
}
...
\begin{itemize}
    \item[\textbullet] Photometry
        \begin{itemize}
            \item[\textbullet] Classify planets using Kopparapu bins \parencite{Kopparapu2018b}
            \item[\textbullet] Use different photometry calculations based on planet classification, likely
                the color classification grid from \textcite{Batalha2018a} for Jovian planets,
                investigate reliable rocky planet photometry models
        \end{itemize}
    \item[\textbullet] Failure mode improvements
        \begin{itemize}
            \item[\textbullet] Want to eliminate ``no failure, always detectable''
        \end{itemize}
    \item[\textbullet] Generalize multivariate Gaussian
        \begin{itemize}
            \item[\textbullet] Increase variation for low ratios
            \item[\textbullet] Potentially incorporate random sampling
        \end{itemize}
\end{itemize}
}

\frame{
\frametitle{Validation with EXOSIMS}
\begin{itemize}
    \item[\textbullet] Create EXOSIMS SimulatedUniverse module that provides synthetic RV data that can be
        fitted with RadVel
    \item[\textbullet] Implement the generalized construction method in EXOSIMS
    \item[\textbullet] Use the $P_{\textrm{det}}$ metric in the target list cost function and
        simulate missions to quantify the effect of this process on exoplanet yield
\end{itemize}
}
...
\begin{itemize}
    ...
    \item[\textbullet] Need to differentiate between star system probability of detection and
        individual planet probability of detection
...
\frame{
    \frametitle{Coursework and TA}
    \begin{itemize}
        \item[\textbullet] MAE 5780 - Feedback Control Systems
        \item[\textbullet] MAE 5730 - Intermediate Dynamics and Vibrations
        \item[\textbullet] MAE 6700 - Advanced Dynamics
        \item[\textbullet] ECE 5210 - Theory of Linear Systems
        \item[\textbullet] (TA) MAE 5730 - Feedback Control Systems
        \item[\textbullet] MAE 6579 - Advanced Astrodynamics
        \item[\textbullet] MAE 6810 - Methods of Applied Math I
        \item[\textbullet] MAE 6780 - Multi-variable Control Theory
        \item[\textbullet] ASTRO 6525 - Multi-wavelength Astronomy (In progress)
    \end{itemize}
}
...
\frame{
    ...
    \begin{itemize}
        \item[\textbullet] Funded by NASA Grant Nos. NN16PJ24C (Science Investigation Team of the
            Nancy Grace Roman Space Telescope Coronagraphic Instrument)
        \item[\textbullet]  Funding through NASA Cooperative Agreement awarded to the New York Space Grant Consortium
        \item[\textbullet] Astropy (astropy.org)
        \item[\textbullet] RadVel \parencite{Fulton2018}
        \item[\textbullet] Imaging Mission Database (plandb.sioslab.com)
        \item[\textbullet] EXOSIMS contributors: Christian Delacroix, Daniel Garrett, Dean
            Keithly, Corey Spohn, Gabriel Soto, and Dmitry Savransky, with contributions by Rhonda Morgan,
            Michael Turmon, Walker Dula, Patrick Lowrance, Neil Zimmerman, and Ewan Douglas.
    \end{itemize}
}
...
\frame[label=coronagraphbackup]{
    \frametitle{Lyot Coronagraph}
\framesubtitle{ \hyperlink{geoconstraint}{\beamerreturnbutton{Return}} }
\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
    \begin{itemize}
        \item[\textbullet] Coronagraph mask sets IWA and OWA
        \item[\textbullet] Deformable mirrors for wavefront control set the largest control radius
    \end{itemize}
};
\tikzoverlay[text width = 14cm] at (2cm, 1.5cm+4.5pt){
    \includegraphics[width=10cm]{figures/APLC.png}
    \begin{tiny}
        \begin{center}
            \textcite{NDiaye2015}
        \end{center}
...
%\tikzoverlay[text width = 14cm] at (6cm, 0cm+4.5pt){
    %\includegraphics[width=5cm]{figures/occultingmask.jpeg}
    %\begin{tiny}
        %\begin{center}
            %jpl.nasa.gov
        %\end{center}
...
\frame[label=radvelbackup]{
    \frametitle{RadVel likelihood}
\framesubtitle{ \hyperlink{radvelmain}{\beamerreturnbutton{Return}} }
\tikzoverlay[text width = 14cm] at (0cm, 3.1cm+4.5pt){
    \begin{align*}
        v_r &= K \left[ \cos\left( \nu +\omega_s \right) +e \cos\left( \omega_s \right)  \right] \\
        \mathcal{V}_r &= \sum_{k}^{N_{\textrm{pl}}} v_{r,k} + \gamma + \dot{\gamma }\left( t-t_0
        \right) + \ddot{\gamma}\left( t-t_0 \right)^2 \\
            \ln \mathcal{L}_i \left( \theta | \mathcal{D}_i \right) &= -\frac{1}{2}\sum_{j}^{}
        \frac{\left( \mathcal{V}_{r,j}\left( t, \theta  \right) -d_j \right)^2}{e_j^2+\sigma_i^2} -
        \ln \sqrt{2 \pi \left( e_j^2 + \sigma_i^2 \right) } 
    \end{align*}
    \begin{itemize}
        \item[\textbullet] $\theta $ - Vector of Keplerian parameters
        \item[\textbullet] $\mathcal{D}_i$ - Data set
        \item[\textbullet] $d_j$ - Each RV measurement
        \item[\textbullet] $e_j$ - Measurement uncertainty associated with each measurement
        \item[\textbullet] $\sigma_j$ - Gaussian noise term to account for
            astrophysical/instrumental noise 
    \end{itemize}
};
}

\frame[label=radvelbackup2]{
    \frametitle{RadVel MCMC}
\framesubtitle{ \hyperlink{radvelmain}{\beamerreturnbutton{Return}} }
\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
    \begin{itemize}
        \item[\textbullet] Utilizes \code{emcee} python package
        \item[\textbullet] Affine invariant ensemble sampling algorithm\textcite{Goodman2010}
    \end{itemize}
};
}

\frame[label=lambertbackup]{
    \frametitle{Lambertian scattering}
    \framesubtitle{ \hyperlink{lambert}{\beamerreturnbutton{Return to constraint}}
    \hyperlink{lambert2}{\beamerreturnbutton{Return to planet elements}} }
\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
    \begin{itemize}
        \item[\textbullet] A Lambertian surface has the same radiance (or flux reflected by a
            surface) when viewed from any angle. Meaning apparent brightness is constant over its
            illuminated surface.
        \item[\textbullet] Phase function: $\Phi(\beta) = \frac{1}{\pi }\left( \sin \beta +\left(
            \pi -\beta \right) \cos \beta  \right) $
        \item[\textbullet] Flux ratio: $F_R = p \Phi\left( \beta  \right) \left( \frac{R_p}{r}^2 \right) $
        \item[\textbullet] $p$ - Ratio of reflected planetary flux at full phase to the flux from a
            perfect lambert disk \textcite{Batalha2018a}
    \end{itemize}
};
\tikzoverlay[text width = 14cm] at (4cm, -1cm+4.5pt){
    \includegraphics[width=6cm]{figures/lambert/phi.png}
};
}
...
\frame[label=intermittent_failure_rules]{
    \frametitle{Intermittent failure rules}
    \framesubtitle{\hyperlink{intermittent_failure_main}{\beamerbutton{Return}}}
\tikzoverlay[text width = 14cm] at (0cm, 3cm+4.5pt){
\begin{enumerate}
    \item[1)] The true planet is not detectable;
    \item[2)] The estimated probability of detection is in the top 90\% of the local probability of
